/**
  ...
  */
#ifndef XT_IR_SEND_H__
#define XT_IR_SEND_H__

//########################################################################################################
#include "application.h" /////////////////// <- 按规定[application.h]作为工程所有软件资源管理头文件！   ##
#ifdef __XT_IR_SEND_REMAP_H //////////////// <- 如果在[application.h]定义此宏，则启动头文件重映射←┐   ##
#include "xt_ir_send_remap.h" ////////////// <- 当启动重映射,则头文件包含将映射到本行定义的头文件←┘   ##
#else //##################################################################################################
...
#endif  //#ifdef __XT_IR_SEND_REMAP_H

#endif  //#ifndef XT_IR_SEND_H__
